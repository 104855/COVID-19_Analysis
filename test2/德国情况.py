...
from pyecharts.charts import Line3D  #导入折线图3D
...
def read_csv_excel():
    ...
    Germany_data=country_T_D_data.loc[57988:58762]   #提取德国数据
    return Germany_data
...
if __name__ == '__main__':
    ...
    Germany_data=read_csv_excel()   # 中国的数据
    #print(Germany_data)
    ...
    total_cases=[]    #总感染人数
    #日期  接种疫苗总数
    for date, total_vaccinations1 in zip(Germany_data['date'], Germany_data['total_vaccinations']):
        date1.append(date)
        total_vaccinations.append(total_vaccinations1)

    for case in  Germany_data['stringency_index']:
        stringency_index1.append(case)
    for icu_patients1 in Germany_data['icu_patients']:
        icu_patients.append(icu_patients1)
    for hosp_patients1 in Germany_data['hosp_patients']:
        hosp_patients.append(hosp_patients1)
    for total_tests1 in Germany_data['total_tests']:
        total_tests.append(total_tests1)

    for total_cases1 in Germany_data['total_cases']:
        total_cases.append(total_cases1)

    total_deaths = []  # 死于 COVID-19 的总死亡人数。包括可能的死亡人数。
    for total_death in Germany_data['total_deaths']:
        total_deaths.append(total_death)

    total_cases = []  # 总感染人数
    for case in Germany_data['total_cases']:
    ...
    L1 = (
        ...
        Line()
            ...
            title_opts=opts.TitleOpts(title="德国"),
      ...
      # 生成line图
    L1.render('C:\\Users\\Bookcatring\\Desktop\\德国情况Line图.html')  # 柱状图   图像展示
