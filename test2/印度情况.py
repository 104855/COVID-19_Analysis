...
def read_csv_excel():
    ...
    India_data=country_T_D_data.loc[70255:71026]   #提取印度数据
    return India_data
...
if __name__ == '__main__':
    ...
    India_data=read_csv_excel()   # 中国的数据
    #print(India_data)
    ...
    total_cases=[]    #总感染人数
    #日期  接种疫苗总数
    for date, total_vaccinations1 in zip(India_data['date'], India_data['total_vaccinations']):
        date1.append(date)
        total_vaccinations.append(total_vaccinations1)

    for case in  India_data['stringency_index']:
        stringency_index1.append(case)
    for icu_patients1 in India_data['icu_patients']:
        icu_patients.append(icu_patients1)
    for hosp_patients1 in India_data['hosp_patients']:
        hosp_patients.append(hosp_patients1)
    for total_tests1 in India_data['total_tests']:
        total_tests.append(total_tests1)

    total_deaths = []  # 死于 COVID-19 的总死亡人数。包括可能的死亡人数。
    for total_death in India_data['total_deaths']:
        total_deaths.append(total_death)

    total_cases = []  # 总感染人数
    for case in India_data['total_cases']:
    ...
    L1 = (
      ...
      # 生成bar图
    L1.render('C:\\Users\\Bookcatring\\Desktop\\印度情况Line图.html')  # 柱状图   图像展示
