...
def read_csv_excel():
    ...
    Japan_data=country_T_D_data.loc[77747:78526]   #提取日本数据
    return Japan_data
...
if __name__ == '__main__':
    ...
    Japan_data=read_csv_excel()   # 中国的数据
    #print(India_data)
    ...
    total_cases=[]    #总感染人数
    total_deaths=[]   #每 100 万人死于 COVID-19 的总死亡人数。包括可能的死亡人数。

    #日期  接种疫苗总数
    for date, total_vaccinations1 in zip(Japan_data['date'], Japan_data['total_vaccinations']):
        date1.append(date)
        total_vaccinations.append(total_vaccinations1)

    for case in  Japan_data['stringency_index']:
        stringency_index1.append(case)
    for icu_patients1 in Japan_data['icu_patients']:
        icu_patients.append(icu_patients1)
    for hosp_patients1 in Japan_data['hosp_patients']:
        hosp_patients.append(hosp_patients1)
    for total_tests1 in Japan_data['total_tests']:
        total_tests.append(total_tests1)

    total_deaths = []  # 死于 COVID-19 的总死亡人数。包括可能的死亡人数。
    for total_death in Japan_data['total_deaths']:
        total_deaths.append(total_death)

    total_cases = []  # 总感染人数
    for case in Japan_data['total_cases']:
    ...
    L1 = (
        ...
        Line()
            ...
            #.add_yaxis("死于COVID-19的比例", total_deaths_per_millions, label_opts=opts.LabelOpts(is_show=False))
            #.add_yaxis("日期", date_list)
            .set_global_opts(
            #xaxis_opts=opts.AxisOpts(axislabel_opts=opts.LabelOpts(rotate=-15)),
            title_opts=opts.TitleOpts(title="日本"),
            #brush_opts=opts.BrushOpts(),  工具选择
      ...
      # 生成bar图
    L1.render('C:\\Users\\Bookcatring\\Desktop\\日本情况Line图.html')  # 柱状图   图像展示
