...
if __name__ == '__main__':
    ...
    #日期  接种疫苗总数
    for date, total_vaccinations1 in zip(China_data['date'], China_data['total_vaccinations']):
        date1.append(date)
        total_vaccinations.append(total_vaccinations1)

    for case in  China_data['stringency_index']:
        stringency_index1.append(case)
    for icu_patients1 in China_data['icu_patients']:
        icu_patients.append(icu_patients1)
    for hosp_patients1 in China_data['hosp_patients']:
        hosp_patients.append(hosp_patients1)
    for total_tests1 in China_data['total_tests']:
        total_tests.append(total_tests1)

    total_deaths = []  # 死于 COVID-19 的总死亡人数。包括可能的死亡人数。
    for total_death in China_data['total_deaths']:
    ...
    L1 = (
        ...
        Line()
            ...
            .add_yaxis("疫苗接种人次", total_vaccinations, label_opts=opts.LabelOpts(is_show=False))
            ...
            #.add_yaxis("当天患者总人数", hosp_patients, label_opts=opts.LabelOpts(is_show=False))
            #.add_yaxis("新冠肺炎总测试人数", total_tests, label_opts=opts.LabelOpts(is_show=False), color='pink')
    ...
    # 生成bar图
    L1.render('C:\\Users\\Bookcatring\\Desktop\\中国情况Line图.html')  # 柱状图   图像展示
